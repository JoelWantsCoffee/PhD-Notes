...
> [!lemma]
> If $\A$ is preabelian then so is $\mathsf{Filt}(\A)$
...
> [!lemma]
> When the underlying category $\A$ is abelian, the two definitions of cokernel agree on induced filtrations. That is, 
...
First, define $\sigma : \mathsf{Filt}(\A) \to \mathsf{Filt}(\A)$ by $\sigma(A_0 \hookleftarrow \cdots) = A_0 \hookleftarrow A_0 \hookleftarrow \cdots$, that is, repeat the first item of the sequence with the identity. A few fun facts.
1. $\sigma$ preserves kernels and cokernels
2. $\sigma$ is additive
3. there's a natural transformation $i : 1 \to \sigma$ 

Let's generalise for a minute.

> [!def]
> Given a category $\A$, an endofunctor $\sigma : \A \to \A$, and a natural transform $i : 1 \to \sigma$, define a new category $\A_\sigma$ as follows.
> - objects: just the objects of $\A$
> - morphisms: $\mathrm{Hom}_{\A_\sigma}(A,B) = \displaystyle\lim_{\xrightarrow[k \geq 0]{}} \mathrm{Hom}_\A(A,\sigma^k B) = \prod_{k \geq 0} \mathrm{Hom}_\A(A,\sigma^k B)\; \big/ \sim$ where $f \sim g$ if for some $p,q \in \mathbb{N}$, $i^pf=i^qg$.

> [!lemma]
> Suppose a functor $F : \A \to \A$ and transform $\eta : F\sigma \to \sigma F$ are such that $i_F\eta = \eta Fi$. Then there is a functor $F_\sigma : \A_\sigma \to \A_\sigma$ with $F_\sigma I \cong I F$.

***proof.*** First we take $F_\sigma = F$ on objects. We can't quite pull the same trick on objects because the types don't line up. Define $F[ f : A \to \sigma^n B ] = [ \eta^n Ff ]$, so that (if it is well defined) it has the desired property. $$ F_\sigma I f = F_\sigma [ f : A \to \sigma^0 B ] = [ \eta^0 Ff ] = IFf $$
It remains to show $f$ is well defined. We need to show that $g \in [f]$ implies $\eta^n Fg \in F_\sigma[f]$.
<iframe class="quiver-embed" src="https://q.uiver.app/#q=&embed" width="100%" height="300" style="border-radius: 8px; border: none;"></iframe>

> [!lemma] 
> Suppose $\A$ is preabelian and $\sigma$ is additive and preserves kernels and cokernels. Then $A_\sigma$ is preabelian, and the obvious inclusion $I : A \to A_\sigma$ is additive and preserves kernels and cokernels.
